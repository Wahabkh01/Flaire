...
export default function CampaignDetailPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Fetch campaign
        const campaignRes = await fetch(`https://emailmarketingbackend.onrender.com/campaigns/${id}`, {
        ...
        if (['sent', 'sending', 'completed'].includes(campaignData.status)) {
          const analyticsRes = await fetch(`https://emailmarketingbackend.onrender.com/campaigns/${id}/analytics`, {
            headers: { Authorization: `Bearer ${token}` },
          });
          if (analyticsRes.ok) {
  ...
  useEffect(() => {
    const fetchLists = async () => {
      try {
        const token = localStorage.getItem("token");
        const res = await fetch("https://emailmarketingbackend.onrender.com/contacts/lists", {
          headers: { Authorization: `Bearer ${token}` },
        });
    ...
    const fetchContacts = async () => {
      try {
        ...
        const res = await fetch(
          `https://emailmarketingbackend.onrender.com/contacts/by-list/${selectedList}`,
          { headers: { Authorization: `Bearer ${token}` } }
        );
        const data = await res.json();
        setContacts(
  ...
  const handleUpdate = async () => {
    try {
      ...
      const res = await fetch(`https://emailmarketingbackend.onrender.com/campaigns/${id}`, {
        method: "PUT",
        headers: {
