...
export default function ListsPage() {
  ...
  const handleRename = async (oldName: string) => {
    ...
    try {
        const res = await fetch(
            `https://emailmarketingbackend.onrender.com/contacts/lists/${encodeURIComponent(oldName)}`, 
            {
              method: "PUT",
              headers: {
  ...
  const handleDelete = async (name: string) => {
    ...
    try {
      const res = await fetch(`https://emailmarketingbackend.onrender.com/contacts/lists/${name}`, {
        method: "DELETE",
        headers: { Authorization: `Bearer ${token}` },
      });
