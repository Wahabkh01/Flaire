...
export function personalizeBody(template: string, contact: any) {
  ...
  export function injectTrackingPixel(
    ...
    return (
      html +
      `<img src="https://emailmarketingbackend.onrender.com/track/open?campaignId=${campaignId}&email=${encodeURIComponent(
        email
  ...
  export function injectTrackingLinks(
    ...
    return html.replace(
      /href="(.*?)"/g,
      `href="https://emailmarketingbackend.onrender.com/track/click?campaignId=${campaignId}&email=${encodeURIComponent(
        email
      )}&url=$1"`
    );
  }
